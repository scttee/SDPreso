...
function SlideBody({ slide }: { slide: (typeof slides)[number] }) {
  ...
  if (slide.type === 'toggle') return <div className='glass-card rounded-2xl p-6'><div className='mb-5 inline-flex gap-2 rounded-full border border-slate/30 p-1'><button onClick={()=>setInside(true)} className={`px-4 py-2 rounded-full transition ${inside?'bg-[#161a1f] text-[#f2eee7]':'text-[#b7c3d7]'}`}>Inside view</button><button onClick={()=>setInside(false)} className={`px-4 py-2 rounded-full transition ${!inside?'bg-[#161a1f] text-[#f2eee7]':'text-[#b7c3d7]'}`}>Outside view</button></div><div className='grid md:grid-cols-4 gap-3'>{(inside?slide.data.inside:slide.data.outside).map((t:string,idx:number)=><div key={t} className='reveal border border-slate/25 p-4' style={{animationDelay:`${idx*70}ms`}}>{t}</div>)}</div><p className='mt-6 text-[#b7c3d7]'>{slide.data.closing}</p></div>;
  ...
  if (slide.type === 'diamond') { const s=slide.data.stages[stage]; return <div className='glass-card rounded-2xl p-6'><div className='grid grid-cols-2 md:grid-cols-4 gap-6 mb-8'>{slide.data.stages.map((st:any,i:number)=><button key={st.name} onClick={()=>setStage(i)} className={`aspect-square rotate-45 border transition-all duration-300 ${stage===i?'border-[#274d8a] shadow-[0_0_0_2px_rgba(39,77,138,.15)]':'border-slate/30'}`}><span className='-rotate-45 block text-sm'>{st.name}</span></button>)}</div><p className='text-2xl mb-2'>{s.question}</p><p className='text-[#b7c3d7]'>{s.explanation}</p><p className='mt-3 text-[#8a4a36]'>If skipped: {s.risk}</p></div>; }
  ...
  if (slide.type === 'steps') return <ol className='space-y-3'>{slide.data.steps.map((s:string,idx:number)=><li key={s} className='reveal flex gap-4' style={{animationDelay:`${idx*80}ms`}}><span className='text-[#b7c3d7] w-8'>{idx+1}.</span><span>{s}</span></li>)}</ol>;
